...
logger = logging.getLogger("backend.services.adk.evaluation")

EVAL_FILE = Path(__file__).parent.parent.parent.parent / "data" / "adk_metrics.json"
...
class EvaluationManager:
    ...
    def _load_metrics(self) -> dict:
                ...
                with open(EVAL_FILE, "r", encoding="utf-8") as f:
                    ...
                    for key in ["compile_successes", "compile_total", "generations_successes", "generations_total", 
                        ...
                        if key not in data:
                            if "scores" in key:
                                data[key] = []
                            elif "runs" in key and "understanding" not in key:
                                data[key] = {}
                            else:
                                data[key] = 0
                    return data
        ...
        return {
            "compile_successes": 18,
            "compile_total": 20,
            "generations_successes": 9,
            "generations_total": 10,
            "edits_successes": 5,
            "edits_total": 5,
            "prompt_understanding_runs": 10,
