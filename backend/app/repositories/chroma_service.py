...
logger = logging.getLogger("backend.app.repositories.chroma")

DB_DIR = Path(__file__).parent.parent / "rag" / "chroma_store_v2"
...
def save_successful_generation(prompt: str, design_plan: dict, critic_score: float):
    """Stores metadata summary of successful designs into ChromaDB."""
    ...
    try:
        ...
        summary = (
            f"Successfully generated UI for prompt: '{prompt}'. "
            f"Visual style selected is {design_plan.get('design_archetype')}, "
            f"heading font: {design_plan.get('font_heading')}, "
            f"body font: {design_plan.get('font_body')}, "
        ...
        logger.info("Saved successful generation summary to vector db.")
    except Exception as e:
