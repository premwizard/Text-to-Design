...
import { 
  Search, FolderOpen, Clock, MoreVertical, Edit2, Copy, 
  Trash2, Star, ChevronDown, Loader2, Play 
} from 'lucide-react';
...
const FILTERS = ['All', 'Today', 'Last 7 Days', 'Last 30 Days', 'React', 'Next.js', 'Vue', 'Other'];
...
export default function RecentlyGenerated() {
  ...
  const [sortBy, setSortBy] = useLocalStorage('ai_projects_sort', 'Newest First');
  ...
  const handleOpenProject = (project) => {
    // Navigate to the editor with the project loaded
    ...
    if (activeFilter !== 'All') {
      ...
      result = result.filter(p => {
        ...
        if (activeFilter === 'Other') {
          return !['react', 'next.js', 'vue'].includes(p.framework?.toLowerCase());
        }
        return true;
      });
    }
  ...
  const favorites = filteredAndSortedProjects.filter(p => p.is_favorite);
  const regulars = filteredAndSortedProjects.filter(p => !p.is_favorite);

  return (
    <div className="h-screen w-screen bg-app text-zinc-100 flex flex-col overflow-hidden font-sans select-none relative">
      <AnimatedBackground />
      <TopNav loading={false} projectName="Recently Generated" />
            ...
            <div className="max-w-7xl mx-auto space-y-8 pb-20">
              ...
              <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                <div>
                  ...
                  <p className="text-lg text-zinc-400 max-w-2xl">
                    Manage, revisit, and iterate on your previously generated designs.
                  </p>
                </div>
              ...
              {loading ? (
                ...
                <div className="flex flex-col items-center justify-center py-20 px-4 text-center border border-white/5 rounded-3xl bg-zinc-900/30">
                  ...
                  <h3 className="text-xl font-bold text-white mb-2">No designs generated yet</h3>
                  <p className="text-zinc-400 max-w-sm mx-auto mb-8">
                    Start creating your first AI-generated UI design to see it appear here in your history.
                  </p>
                  <button 
